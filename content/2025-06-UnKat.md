...
| Libraries                | Good | The contracts use the OpenZeppelin contracts for main functionalities. The Merkle library is used for compatibility when accessing Merkle rewards. |
...
UnKatVault’s multicall employs a blacklist to block direct calls to core contracts (Merkl Distributor, unKat, Kat token, Factory, itself). The addition of the multicall opens up the potential for issues as user can execute arbitrary calls, and the addition of a base set of blacklisted addresses may not be sufficient to stop this.
